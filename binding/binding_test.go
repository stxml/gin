...
func TestBindingFormWithPointers1(t *testing.T) {
	testFormBindingWithPointers(t, "GET", "/?Int=1&Int8=1&Int16=1&Int32=1&Int64=1&UInt=1&UInt8=1&UInt16=1&UInt32=1&UInt64=1&Bool=true&Float32=0.1&Float64=0.1&String=foo", "")
}

func TestBindingFormWithPointers2(t *testing.T) {
	testFormBindingWithPointers(t, "POST", "/", "Int=1&Int8=1&Int16=1&Int32=1&Int64=1&UInt=1&UInt8=1&UInt16=1&UInt32=1&UInt64=1&Bool=true&Float32=0.1&Float64=0.1&String=foo")
}

func TestFormBindingMixed(t *testing.T) {
	var obj struct {
		Foo *int   `form:"foo"`
		Bar string `form:"bar"`
	}
	req := requestWithBody("GET", "/?foo=1&bar=yes", "")
	err := Form.Bind(req, &obj)
	assert.NoError(t, err)
}

func TestFormBindingWithPointersFails(t *testing.T) {
	var obj struct {
		Foo *int `form:"foo"`
	}
	req := requestWithBody("GET", "/?foo=bar", "")
	err := Form.Bind(req, &obj)
	assert.Error(t, err)
}
...
func TestExistsWithFormSucceeds(t *testing.T) {
	type HogeStruct struct {
		Hoge *int `form:"hoge" binding:"exists"`
	}
...
func TestExistsWithFormFails(t *testing.T) {
	type HogeStruct struct {
		Hoge *int `form:"foo" binding:"exists"`
	}
...
func testFormBindingWithPointers(t *testing.T, method, path, body string) {
	req := requestWithBody(method, path, body)
	if method == "POST" {
		req.Header.Add("Content-Type", MIMEPOSTForm)
	}
